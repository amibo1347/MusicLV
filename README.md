...
| 구분 | 아이디 | 비밀번호 |
|---|---|---|
| 관리자 | `admin@musiclv.com` | `admin1234` |

관리자로 로그인하면 상품 · 공연 등록/수정, 주문 · 예매 상태 관리까지 확인할 수 있다.
구매자 흐름은 회원가입 없이 **비회원 주문**으로도 체험할 수 있고, 주문 후 받은 주문번호와 연락처로 조회하면 된다.

> 위 계정은 `ADMIN_PASSWORD` 환경변수를 지정하지 않았을 때 만들어지는 기본값이다.
> 운영에서 비밀번호를 바꿨다면 이 표도 함께 고쳐야 한다.
